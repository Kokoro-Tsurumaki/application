GC垃圾回收机制为了改进Java不足而设计了[[Kotlin]]语言，面向对象，应用场景广泛，最常用于Web程序开发和[[Android]] APP开发。
### Java三大特征

Java是一个面向对象的高级语言,Java三大特征分别是封装、继承、多态。

封装就是将类的属性私有化，并提供Get、Set方法以操作属性，把对属性的约束统一放在Get、Set中，避免直接操作属性时重复的进行约束，方便进行对属性的约束进行统一的管理，对于只在内部使用的方法可以定义为私有方法，外部无法直接调用，确保数据的安全性。

Java不支持多继承，被继承的类叫父类，继承它的类叫子类，子类继承父类，子类拥有父类的除Private修饰的一切属性和方法 。

多态的本质就是同一行为的不同表现形式，比如子类继承父类，子类重写父类的方法，父类引用指向子类对象就会指定不同子类方法逻辑。重写就是子类对父类某个方法的重新定义，子类重写的方法的参数与返回值必须与父类一致。重载就是在一个类中同一个方法，方法名相同，但是它的参数和返回值不相同。另一种多态就是不同类实现同一个接口，对同一方法的具体实现可能不一致。

隐藏内部实现、提供公共接口
子类继承父类的特征和行为
多态指统一操作作用于不同对象产生不同行为
...
在Android应用中，为了避免内存泄漏，在一些封装类持有的Context一般会使用弱引用，当这个Activity销毁时，不影响对象的回收。
...
在相关场景时可以使用定点整数代替小数或者使用BigDecimal，它采用定点小数表示法(将数字以十进制表示并记录位数信息)和十进制运算，避免了二进制的舍入误差。
...
### List,Set,Map的区别
List：有序，可重复  ，底层实现了一个动态数据，当用完时以1.5倍
