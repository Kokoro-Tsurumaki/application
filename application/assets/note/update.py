...
all_tasks = [
    {
        "ROOT_DIR" : "留档",
        "OUTPUT_FILE" : "留档/留档.md"
    },
    {
        "ROOT_DIR": "study",
        "OUTPUT_FILE": "study/study.md"
    },
    {
        "ROOT_DIR": "CS",
        "OUTPUT_FILE": "CS/cs.md"
    },
]
...
def generate_md_structure(root_dir):
    ...
    print(f"Markdown 结构查询: {os.walk(root_dir)}")

    for root, dirs, files in os.walk(root_dir):
        # 计算当前层级
        level = root.replace(root_dir, "").count(os.sep)
        indent = "  " * (level + 1)  # 增加缩进使其对齐
        print(f"Markdown 当前层级: {level}")
        print(f"Markdown 当前目录: {os.path.basename(root)}")
        # 获取相对路径，并确保保留根目录
...
for current_task in all_tasks:
    # 生成 Markdown 并写入文件
    with open(current_task["OUTPUT_FILE"], "w", encoding="utf-8") as f:
        f.write(generate_md_structure(current_task["ROOT_DIR"]))
    print(f"Markdown 目录已生成: {current_task["OUTPUT_FILE"]}")
