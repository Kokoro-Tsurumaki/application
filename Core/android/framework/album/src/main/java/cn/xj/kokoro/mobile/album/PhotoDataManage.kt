 ...
 * Description:
 */
class PhotoDataManage(private val applicationContext: Context) {
    //可以理解为一个资源管理器
...
//            cursor?.close()
            ...
            lifecycleCoroutineScope.launch(Dispatchers.IO) {
                if (cursor != null){
                    ...
                    while (count < maxCount && cursor!!.moveToNext()){
                        Log.e("TAG", "addPhoto: ${cursor!!.position+1}", )
                        Log.e("TAG", "addPhoto: ${cursor!!.count}", )
                        ...
                        try {
                            ...
                            if (option.outWidth <= 0 || option.outHeight <= 0) continue
                            if (option.outMimeType == null) continue
                            ...
                            val folderKt = Folder.Photo(
                                FileManage.getProviderUri(applicationContext, path),
                                false,
                                -1,
                                size.toLong(),
     ...
     */
     fun addPhoto(uri: Uri,success: () -> Unit){
        ...
        //文件名
        val fileName : String = DocumentFile.fromSingleUri(applicationContext, uri)!!.name!!.split(".")[0]
        val fis: InputStream = applicationContext.contentResolver.openInputStream(uri)!!
        fun updateList(){
            //data数组中是否已经拥有该目录
            var haveFolder = false
            val newUri = FileManage.getProviderUri(applicationContext,"${folderName}/${fileName}.jpg")
            val size = FileManage.getFileSize(applicationContext,newUri)
            val folderKt = Folder.Photo(newUri, false, -1, size, FileManage.format(size), "jpg")
            //无论是否拥有都添加进全部图片中 如果初始化过
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            val uri10 = applicationContext.contentResolver.insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values)
            val fos: OutputStream =   applicationContext.contentResolver.openOutputStream(uri10!!)!!
            FileManage.dataTransfer(fis,fos)
            if (dataList.isNotEmpty()) updateList()
        }else{
            ...
            MediaScannerConnection.scanFile(
                applicationContext, arrayOf("${folderName}/${fileName}.jpg"), arrayOf("image/jpeg")
            ) { _: String, _: Uri ->
