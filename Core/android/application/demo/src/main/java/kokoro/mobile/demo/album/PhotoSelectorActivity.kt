package kokoro.mobile.demo.album
...
import kokoro.mobile.demo.album.list.FolderWindowAdapter
import kokoro.mobile.demo.album.list.PhotoListAdapter
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.GlobalScope
import kotlinx.coroutines.launch
import kotlinx.coroutines.withContext
import kokoro.mobile.demo.R
import kokoro.mobile.demo.base.BaseActivity
import kokoro.mobile.demo.base.BaseBindingActivity
import kokoro.mobile.demo.databinding.ActivityPhotoSelectorBinding
import kokoro.mobile.demo.utils.PopupWindowManager
 ...
 */
fun Context.selectorImage(
    maxChoose: Int = 1,
    enableCompression: Boolean = true,
    useVideo: Boolean = false,
    resultSuccess: ((Array<String>) -> Unit)
) {
...
class ImageSelectorActivity : BaseBindingActivity<ActivityPhotoSelectorBinding>() {
    ...
    private val windowAdapter by lazy {
        FolderWindowAdapter(this, photoDataManage).apply {
            clickListener = {
                gridAdapter.notifyItemRangeRemoved(
                    0,
                    if (choosePosition == 0) photoDataManage.dataList[choosePosition].fileList.size + 1 else photoDataManage.dataList[choosePosition].fileList.size
                )
                choosePosition = it
        ...
        PhotoListAdapter(this@ImageSelectorActivity, photoDataManage).apply {
            ...
            setState = { position ->
                if (photoDataManage.dataList[getChoosePosition?.invoke()!!].fileList[position].state) {
                    photoDataManage.dataList[getChoosePosition?.invoke()!!].fileList[position].state =
                        false
                    for (i in 0 until photoDataManage.dataList[0].fileList.size) {
                        if (photoDataManage.dataList[0].fileList[i].id > photoDataManage.dataList[getChoosePosition?.invoke()!!].fileList[position].id) {
                            ...
                            notifyItemRangeChanged(
                                if (getChoosePosition?.invoke() == 0) i + 1 else i,
                                1
                            )
                        }
                    }
                    photoDataManage.dataList[getChoosePosition?.invoke()!!].fileList[position].id =
                        -1
                    if (photoDataManage.getCurrentChooseSize() == 0) {
                        binding.finishButton.text = "完成"
                        binding.finishButton.isEnabled = false
                    } else {
                        binding.finishButton.text =
                            "完成${photoDataManage.getCurrentChooseSize()}/${maxChoose}"
                    ...
                    if (photoDataManage.getCurrentChooseSize() < maxChoose) {
                        ...
                        photoDataManage.dataList[choosePosition].fileList[position].id =
                            photoDataManage.getCurrentChooseSize()
                        binding.finishButton.text =
                            "完成${photoDataManage.getCurrentChooseSize()}/${maxChoose}"
                        binding.finishButton.isEnabled = true
                    } else Toast.makeText(
                ...
                notifyItemRangeChanged(
                    if (getChoosePosition?.invoke() == 0) position + 1 else position,
                    1
                )
            }
        }
    ...
    override fun getViewId(): Int = R.layout.activity_photo_selector
            ...
            registerForActivityResult(ActivityResultContracts.TakePicture()) {
                if (it) {
                    photoDataManage.addPhoto(uri!!) {
                        ...
                        gridAdapter.notifyItemRangeChanged(
                            1,
                            photoDataManage.dataList[0].fileList.size
                        )
                    }
                }
            }
