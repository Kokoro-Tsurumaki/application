...
import kokoro.mobile.template.BuildTypeConstant.BASE_URL
...
class LogInterceptor : Interceptor {
    override fun intercept(chain: Interceptor.Chain): Response {
        _root_ide_package_.kokoro.mobile.template.model.NetLogModel().apply {
            val request = chain.request()
            val requestBody = request.body
            val connection = chain.connection()
            //--> PoST /greeting http/1.1
            requestStartMessage =
                ("${request.method} ${request.url.toString().removePrefix(_root_ide_package_.kokoro.mobile.template.BuildTypeConstant.BASE_URL)}${if (connection != null) " " + connection.protocol() else ""}")
...
//                val value = if (headers.name(i) in headersToRedact) "██" else headers.value(i)
            ...
            responseStartMessage =
                "${response.code}${if (response.message.isEmpty()) "" else ' ' + response.message} ${request.url.toString().removePrefix(
                    _root_ide_package_.kokoro.mobile.template.BuildTypeConstant.BASE_URL
                )} (${tookMs}ms)"
