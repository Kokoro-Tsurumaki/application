...
class GlobalExceptionHandler {
    ...
    fun handleHttpMessageNotReadable(
        ...
        return ResponseEntity<Result>(errorResponse, HttpStatus.OK)
    }

    @ExceptionHandler(CannotGetJdbcConnectionException::class)
    fun handleCannotGetJdbcConnectionNotReadable(
        ex: HttpMessageNotReadableException, request: WebRequest
    ): ResponseEntity<Result> {
        // 分析异常原因
        ...
        println("root 不为空")
        if (ex.message!!.contains("Failed to obtain JDBC Connection")) {
            errorMessage = ResultCode.CUSTEM(500, "数据库连接异常")
        } else{
            errorMessage = ResultCode.CUSTEM(400, ex.message?:"message be blank")
        }
    ...
    fun handleAllExceptions(ex: Exception?, request: WebRequest): ResponseEntity<Result> {
        return ResponseEntity<Result>(Result.error(ResultCode.ERROR), HttpStatus.OK)
    }
}